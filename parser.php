<?php   //  ۞ // text { encoding:utf-8 ; bom:no ; linebreaks:unix ; tabs:4sp ; }
									$cbp_version = '1.3.1'; // [HTML5 compliant]
/*
	...
	cbparser.php
	the corzblog bbcode to html and back to bbcode parser
	...
	HTML5. We use css to style the output as desired, your call.
	...
	cbguide also does cool JavaScript buttons for common stylings, and links to
	the bbcode guide, for users' convenience.
	...
	it will return nothing. A global variable..
	$GLOBALS['cbparser']['warning_message'], or $cb_warning_message, if you
	prefer, will be available with the message "spammer". You can catch that,
...
// path to blog..
// if running inside corzblog, this will already be set..
if (!isset($corzblog['blogzpath'])) { $corzblog['blogzpath'] = '/cbparser/'; }
...
*/
if (empty($smiley_folder)) { $smiley_folder = $corzblog['blogzpath'].'img/smileys/'; }
/*
...
$cb_guide_path = $_SERVER['DOCUMENT_ROOT'].$corzblog['blogzpath'].'cbguide.php';
...
// we check for multiple pajamas-enabled scripts that might be using cbparser..
//
switch (true) {

	case isset($GLOBALS['corzblog']['use_pajamas']):
		$use_pajamas = $GLOBALS['corzblog']['use_pajamas'];
		break;

	case isset($GLOBALS['comment_thing']['use_pajamas']):
		$use_pajamas = $GLOBALS['comment_thing']['use_pajamas'];
		break;
...
$GLOBALS['cbparser']['warnings']['imbalanced'] = '
		...
		<div id="bbinfo">
			...
			To insert <strong><a href="http://corz.org/serv/tricks/htaccess.php" title="tricks with Apache server" id="link-corz-htaccess-tutorials">.htaccess</a> code</strong>, shell code or ascii art, use <code><strong>[pre][/pre]</strong></code> tags.<br /><pre>
[pre]
...
// legacy HTML
//
// If you have HTML created with a *very* old version of cbparser, and want those converted into
// nice, clean HTML5-capable bbcode in the html2bb() process, enable this (set to true)..
//
$legacy_bbcode = false;
...
//		$tmp_str = highlight_string(stripslashes($tmp_str), true);
	...
	if ($GLOBALS['trans_warp_drive']) { $GLOBALS['cbparser']['text'] = strrev($GLOBALS['cbparser']['text']); }
...
$html2bb = func_get_arg(0);
	...
	// uncomment this if you need to convert really old cbparser-created html documents into new bbcode
	if ($GLOBALS['legacy_bbcode']) {
		if (stristr($html2bb, '<font') or stristr($html2bb, 'align=')  or stristr($html2bb, 'border=') or stristr($html2bb, '<i>')
										or stristr($html2bb, 'target=') or stristr($html2bb, '<u>') or stristr($html2bb, '<br>')) {
...
<div class="reftext">';
	...
	$html2bb = str_replace('</big>', '[/big]', $html2bb);	// legacy catcher
	$html2bb = str_replace('<span class="big">', '[big]', $html2bb);
...
global $smiley_folder;
	...
	$html2bbtxt = str_replace(']', '**@^@**', $html2bbtxt);
	$html2bbtxt = str_replace('<table width="20%" border="0" align="right"><tr><td align="center"><span class="news"><b><span class="big">', '[news]', $html2bbtxt);
	$html2bbtxt = str_replace('</span></b></span></td></tr></table>', '[/news]', $html2bbtxt);
...
<font class="ref"><b><u><a href="javascript:history.go(-1)">references:</a></u><br><br>1: </b></font><font class="reftext">';
	...
	$html2bbtxt = str_replace('</u>', '[/u]', $html2bbtxt);
	$html2bbtxt = str_replace('<span class="big">', '[big]', $html2bbtxt);
	$html2bbtxt = str_replace('<!--big--></span>', '[/big]', $html2bbtxt);
	...
	// legacy "smilie" locations..
	if (stristr($html2bbtxt, 'smilie for ')) {
...
function xssclean($string) {
	...
	// style expression hacks. only works in buggy ie... (fer fuxake! get a browser!) no need for these now - all style tags are killed
...
function do_bb_form() {
global $auth, $corzblog, $spelling, $use_pajamas;
...
/*
	bbcode to lowercase.	[updated: 12-08]
...
if (realpath($_SERVER['SCRIPT_FILENAME']) == realpath(__FILE__)) { // direct access. do the demo.
	if (strstr($_SERVER['HTTP_HOST'], 'corz.')) { // for the mama cbparser!
		include_once($_SERVER['DOCUMENT_ROOT'].'/blog/init.php');	// just for my footer image location
...
[big-spoiler]oh, and I capitulated on the color tags, [color=red]here[/color] [color=blue]you[/color] [color=#C5BB41]go..[/color] [color=pink]you can use any of the "named" colour values, like this pink here,[/color] [color=#9C64CA]or a proper hex color value, the best of both worlds.[/color][/big-spoiler]
...
<head>
<meta charset="utf-8">
<title>corzblog bbcode to html to bbcode parser (free, php) built-in demo - v'.$cbp_version.'</title>
...
<style media="screen">
/*<![CDATA[*/
@import "http://corz.org/inc/css/main.css";
@import "http://corz.org/blog/inc/themes/original/original.css";
.hidden { display: none; }
/*]]>*/
</style>
...
<body>';
	...
	if (file_exists($_SERVER['DOCUMENT_ROOT'].'/public/machine/download/php/corz function library/corzblog.bbcode.parser.php.zip')
		or (!stristr($_SERVER['HTTP_HOST'], 'corz.'))) {
		echo '
...
/*
	...
	or why it stopped happening..
		...
		*	HTML5 output (inc. replacement tags for <tt>, etc.)


		1.2.9

		*	Fixed a bug in the [ccc] tag where editing would produce strange chatacters
			(these were "A0", aka. "non breaking space" characters which were being converted
			internally by php).
		...
		+	Added the [spoiler][/spoiler] tag. Text inside this tag is replaced by
			an image which reads "Spoiler!". If the user hovers the mouse over the
			image, it disappears, revealing the text beneath. This was conceived for
			writing reveiws of material in places where someone who has not *yet*
			seen the material, may be passing by. e.g. film critique.

			There's also a [big-spoiler][/big-spoiler] variant, which creates a
			<div>. It will fill whatever width it is given, and you can put block-
			level tags; like [pre] tags, and big boxes; inside it.

		1.2.5

		*	Fixed a silly error in the legacy "smilie" detection, where it would
			switch to legacy mode if the word "smilie" was in the post. But
			people actually use this string, regarldess of it being a misspell!
			The detection is now slightly more specific.
		...
		*	Added an improved [quote] tag. the old <cite> tags will be
			...
			I use a background image (in my css) for the cute new [quote] tags,
			so edit that into your css - grab my css for an example - though
			unless you are generating css, you probably want to grab the IE
			version, with the .gif, as opposed to the proper version, with a
			png. you can't js hack png transparency for css images - sadly.

			When the new corzblog alpha/beta comes out, you can grab my
			css-generating code, which automatically switches the layout
			depending on the browser; through it's easy enough to cook up your
			own, if you know a little php.

			Here, roughly, is the code I use to get the new quote tags looking
			neat on all platforms  - we're inside an echo '' here, with $browser
			[user-agent string token] already established..

				.quote {
					min-height: 50px;';
				if (substr($browser, 0, 2) == 'ie') { echo '
					background: transparent url(/img/gif/quote.gif) no-repeat;
					height: 50px;
					display: block;
					overflow: visible;
					'; } else { echo '
					background: transparent url(/img/quote.png) no-repeat;';} echo '
			...
			with short quotes (2 lines or less - you see this all over the web)
			so this keeps the whole thing visible, regardless of the size of
			the quote.
